...
class CloudWatchBatcher:
    """Batches GetMetricData queries (<=500 per request), with pagination + retry."""
    def __init__(self, region: str, client: Optional = None):
        ...
        self._mdqs: List[MetricQuery] = []

    def add(self, q: MetricQuery) -> None:
        self._mdqs.append(q)

    def extend(self, qs: Iterable[MetricQuery]) -> None:
        self._mdqs.extend(qs)

    def _to_query_dict(self, q: MetricQuery) -> Dict[str, Any]:
        # Accept MDQ dataclass OR a prebuilt GetMetricData query dict
        if isinstance(q, MDQ):
            return {
                "Id": q.id,
                "MetricStat": {
        ...
        # Assume it's already a dict shaped for GetMetricData
        return q
    ...
    def execute(self, start: datetime, end: datetime, scan_by: str = "TimestampDescending") -> Dict[str, List[Tuple[datetime, float]]]:
        ...
        for i in range(0, len(self._mdqs), 500):
            ...
            queries = [self._to_query_dict(q) for q in chunk]

            next_token = None
            while True:
                kwargs = dict(MetricDataQueries=queries, StartTime=start, EndTime=end, ScanBy=scan_by)
                if next_token:
